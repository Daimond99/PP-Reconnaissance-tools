...
def main():
    ...
    # Keep the splash up past widget construction, until the LLM Mode
    # terminal has actually spawned its WSL/bash backend (not just until
    # Python-side __init__ returns) — WSL's own boot can take a few seconds
    # longer than building the widget tree. The Wizard Console no longer
    # opens a terminal of its own (it's a hidden `--gui` subprocess driven
    # entirely by Qt dialogs, see `src/ui/wizard_runner.py`), so
    # `opencode_tab` (opened up front, `fixed=True`) is the readiness proxy.
    closed = {"done": False}
    ...
    window.main_area.opencode_tab.firstTabReady.connect(_close_splash)
