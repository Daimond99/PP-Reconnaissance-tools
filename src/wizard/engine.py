...
Orchestration only: menu/help text and scan profiles live under
src/resources/ and are loaded through the Resource Loader; validation,
command building, and impact analysis are delegated to their own layers
(docs/Wizard Engine Architecture.md #16).
"""
...
_MENU = load_json("wizard/menu.json")
_MESSAGES = load_json("wizard/messages.json")
_WARNINGS = load_json("common/warnings.json")
_SCAN_PROFILES = load_json("nmap/scan_profiles.json")
_SUGGESTION_REASONS_RAW = _MESSAGES.get("suggestion_reasons", {})

AUTHORIZED_SCOPE_WARNING = _WARNINGS.get("authorized_scope_warning", "")

DEMO_TOOL_RESTRICTION_NOTICE = _MESSAGES.get("demo_tool_restriction_notice", "")

CHAIN_DISABLED_NOTICE = _MESSAGES.get("chain_disabled_notice", "")
...
# ---------------------------------------------------------------------------
# Scan Level definitions — grouped by AttackType, sourced from
# resources/nmap/scan_profiles.json
...
def _scan_levels_for(attack: AttackType) -> List[ScanLevelDef]:
    ...
    stealth = [tuple(item) for item in _SCAN_PROFILES.get("stealth", [])]
    verbose = [tuple(item) for item in _SCAN_PROFILES.get("verbose", [])]
    aggressive = [
        tuple(item)
        for item in _SCAN_PROFILES.get("aggressive", {}).get(attack.value, [])
    ]
...
def _get_base_ports_for(attack: AttackType) -> str:
    ...
    return _SCAN_PROFILES.get("base_ports", {}).get(attack.value, "")


def _suggestion_reason(attack: AttackType) -> str:
    return _SUGGESTION_REASONS_RAW.get(attack.value, "")
...
class WizardEngine:
    ...
    TOOL_MENU = _MENU.get("tool_menu", "")
    ATTACK_MENU = _MENU.get("attack_menu", "")
    DB_OPTIONS = _MENU.get("db_options", "")
    ...
    def _goto_suggestion(self, prefix_lines: List[str]) -> WizardResult:
        ...
        attack = self.state.attack_type
        label = attack.value.replace("_", " ").title()
        reason = _suggestion_reason(attack)
