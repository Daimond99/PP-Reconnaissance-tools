...
- `src/ui/` — `main_window.py` (layout), `widgets.py` (Sidebar/TopBar/Console), `terminal.py` (plain-pipe `InteractiveTerminal`), `pty_terminal.py` (ConPTY + pyte). The dead UI pages `tool_selection.py`/`llm_mode.py` and the old native wizard pages `wizard_terminal.py`/`wizard_console.py` + `src/wizard/engine.py` were all removed — the Wizard Console page now embeds the standalone `chain_wizard/` chain CLI via `src/ui/pty_terminal.py`. See `docs/CROSS_PLATFORM_TERMINAL_PLAN.md`.
