...
**All 6 tools execute inside a sandboxed Docker container (`therecon-tools`), never directly on the WSL2/Windows host.** `docker/Dockerfile` builds the image (non-root user, scoped NOPASSWD sudo for exactly `nmap`/`masscan`); `docker/run.sh` is the *only* place `docker run` is ever invoked (no other code path can smuggle in an unsafe flag) — `--cap-drop=ALL` + a minimal cap-add, `--read-only` rootfs, no `docker.sock`/`/mnt/c` mount, a dedicated bridge network (not `--network=host`). The app talks to it via `docker exec` (`chain_wizard/core/executor.py`, `src/ui/terminal_launch.py`) instead of a bare `subprocess.run(shell=True)` on the host. This exists specifically so that an indirect prompt-injection scenario (a scanned target's banner tricking an LLM into suggesting a `sudo`-prefixed command that a human then approves) still only ever damages the disposable container, never the real host — see `docs/DOCKER_SANDBOX_DEFENSE.md` for the full threat model, design, and live-tested verification.
...
`docs/ARCHITECTURE.md`, `docs/Resource System.md`, `docs/AI_DEVELOPMENT.md`, and `docs/Coding Standards.md` (the original normative specs) have been removed — their load-bearing rules are folded into this file's `## Architecture` section below and into the docs that remain. **Start with `docs/CURRENT_STATE.md`** — a snapshot of what's actually implemented vs. placeholder, including live safety gaps and cross-platform terminal design (read this first in a fresh session — cheaper than re-deriving it). The rest of `docs/` (kept deliberately small — see its own README-style index):
- `docs/DOCKER_SANDBOX_DEFENSE.md` — the container sandbox's threat model, design, and live test evidence.
- `docs/WIZARD_DATA_FLOW.md` — `chain_wizard/`'s internal data flow + every JSON schema that crosses a process/storage boundary.
- `docs/OVERVIEW_TH.md` — Thai-language first-read overview (shorter, less exhaustive than this file).
- `docs/PP-scope-checklist.md` / `docs/PP-SCOPE-QA.md` — the academic project-scope checklist and a defend-ready Q&A mapping each scope item to code evidence.
- `docs/PROGRESS.md` — append-only running log of what's been done, session by session; a history, not a spec — don't treat it as current-state truth without cross-checking `CURRENT_STATE.md`.

Key points that aren't obvious from skimming the code:
...
- `src/ui/` — `main_window.py` (layout, title-bar Settings dropdown + sidebar toggle, Direct Tool Mode Execute → Raw Output, Open/Save scan XML, `closeEvent` asks Yes/No then on Windows runs `wsl --shutdown`), `widgets/` (package, one class per module — Sidebar/TopBar/DropdownButton/`InputManagementTab` scan queue/`RawOutputTab`/`ResultsDisplayTab`/`MainContentArea`; `__init__` re-exports so `from src.ui.widgets import …` is unchanged), `terminal.py` (plain-pipe `InteractiveTerminal`), `terminal_tabs.py` (`TerminalTabsWidget` — PyCharm-style tab bar for the Wizard Console; capped at `_MAX_TABS = 4` open at once; `_wsl_available()` shows a plain "install WSL" placeholder instead of a blank terminal when no distro is registered, never hardcoded to a distro name — launches via `wsl.exe` with no `-d` flag so whatever the user's default distro is gets used), `webterm/` (`XtermTerminal` — xterm.js in QWebEngineView + real PTY, the primary terminal backend), `pty_terminal.py` (`PtyTerminal` — pyte + ConPTY, legacy fallback). Sidebar pages are now 5 (Command Editor removed): Wizard Console / Input Management / Raw Output / Results Display / LLM Mode. See `docs/CURRENT_STATE.md`.
- **`chain_wizard/`** (repo root, NOT under `src/`) — the live chain wizard: scan → impact-ranked plan (AUTO only — SEMI removed 2026-08-29) → hydra brute → credential harvest/loot → in-scope post-exploit (ncat/nmap-NSE/evil-winrm). Self-contained Python, 6-tool-restricted, arsenal/post-exploit in JSON. This is the current Wizard Console.
...
`docs/CURRENT_STATE.md`'s "Known gaps & status" section is the single source of truth — read it before pointing this app at a real target. Short version, current as of 2026-09-22: **three** paths now route through `ConfirmationGate` (Direct Tool Mode Execute, Wizard Console per-step confirm, and the gated "LLM Nmap" suggestion panel) — this grew from the original single Direct-Tool-Mode-only gate as the wizard and LLM panel were wired in. The **OpenCode tab** and the plain **Shell tab** are still ungated Qt-side (no `ConfirmationGate`, no exact-`"yes"`), by design — but every command either one runs is contained by the Docker sandbox above regardless, which is the actual backstop for that tab, not the confirmation dialog. `RawOutputTab` stays read-only (no keystroke ever reaches its PTY). `skip_scope=True` is used broadly (Direct Tool Mode and Wizard Console both) — `AUTHORIZED_SCOPE` is enforced only on the AI-suggestion panel's free-typed target, not on paths where the human typed the target themselves; that's an intentional trade-off, not an oversight.
